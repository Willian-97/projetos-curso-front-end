class BoxShadowGenerator {
  constructor(
    ...
    (this.blur = blur), (this.blurRef = blurRef);
    this.spread = spread;
  ...
  applyRule() {
    const regbValue = this.hexToRgb(this.colorRef.value);
    ...
    }px rgba( ${regbValue}, ${this.opacityRef.value})`;
...
const ruleArea = document.querySelector("#rules-area");
const copyInstruction = document.querySelector("#copy-instructions");

ruleArea.addEventListener("click", () => {
  const rules = ruleArea.innerText.replace(/^\s*\n/gm, "");

  navigator.clipboard.writeText(rules).then(() => {
    copyInstruction.innerHTML = "Regra copiada com sucesso!";
    setTimeout(() => {
      copyInstruction.innerText =
